...
export default function Admin() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      setApplications(snapApps.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        // Format with time (e.g. 11/05/2026, 10:30 PM)
  ...
  const isDuplicate = (val, field) => applications.filter(a => a[field] === val).length > 1;

  // College count calculation
  const collegeCounts = applications.reduce((acc, app) => {
    acc[app.college] = (acc[app.college] || 0) + 1;
    return acc;
  }, {});
    ...
    <div className="min-h-screen bg-gray-50 font-body">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-8">
          <input type="text" placeholder="Search Name, Email or College..." value={search} onChange={e => setSearch(e.target.value)} className="md:col-span-2 px-4 py-2.5 border border-gray-200 rounded-sm text-sm focus:ring-1 focus:ring-corp-blue outline-none" />
          <select value={filterBrand} onChange={e => setFilterBrand(e.target.value)} className="px-4 py-2.5 border border-gray-200 rounded-sm text-xs font-bold uppercase tracking-wider bg-white">
            <option>All Brands</option><option>MyTripRaja</option><option>MarketerRaja</option>
          </select>
          <select value={filterYear} onChange={e => setFilterYear(e.target.value)} className="px-4 py-2.5 border border-gray-200 rounded-sm text-xs font-bold uppercase tracking-wider bg-white">
                  ...
                  {filteredApps.map(app => (
                    <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-xs font-mono">
                        ...
                        <div className={`text-[10px] mt-0.5 ${isDuplicate(app.email, 'email') ? 'text-orange-600 font-bold' : 'text-blue-600'}`}>{app.email} {isDuplicate(app.email, 'email') && "⚠️"}</div>
