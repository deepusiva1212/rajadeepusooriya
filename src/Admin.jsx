...
export default function Admin() {
  ...
  const [staffList, setStaffList] = useState([]); // RESTORED
  ...
  const [newStaff, setNewStaff] = useState({ name: "", email: "", role: "User", employeeId: "" }); // RESTORED
  ...
  const [templates, setTemplates] = useState(() => {
    ...
    if (saved) { return { ...defaultTemplates, ...JSON.parse(saved) }; }
  ...
  const fetchAllData = async (role = staffData?.role) => {
    setLoadingData(true);
    try {
      ...
      // RESTORED: Fetch Staff List so the Team Access tab works!
      if (role === "Super Admin") {
  ...
  // RESTORED: Add & Remove Staff Functions
  const addStaffMember = async (e) => {
    ...
    } catch (error) { showToast("Failed to create staff.", "error"); }
  };

  const removeStaffMember = async (staffId) => {
    if (!window.confirm("Permanently revoke this user's access?")) return;
    try {
  ...
  const sendIndividualEmail = (app, isWelcome = false) => {
    ...
    bodyText = bodyText.replace(/\[Name\]/g, app.name || "Candidate").replace(/\[ID\]/g, app.applicationId || "N/A").replace(/\[Brand\]/g, app.brand || "our company").replace(/\[Gender\]/g, app.gender || "N/A").replace(/\[Phone\]/g, app.phone || "N/A").replace(/\[AltPhone\]/g, app.altPhone || "None").replace(/\[University\]/g, app.university || "N/A").replace(/\[College\]/g, app.college || "N/A").replace(/\[Stream\]/g, app.stream || "N/A").replace(/\[Major\]/g, app.major || "N/A").replace(/\[Batch\]/g, app.batch || "N/A").replace(/\[Year\]/g, app.year || "N/A").replace(/\[Duration\]/g, app.duration || "N/A").replace(/\[LinkedIn\]/g, app.linkedin || "Not Provided").replace(/\[Portfolio\]/g, app.portfolio || "Not Provided").replace(/\[Resume\]/g, app.resumeUrl || "Not Attached");
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredApps.map(a => `"${a.applicationId}","${a.dateStr}","${a.batch}","${a.name}","${a.gender}","${a.email}","${a.phone}","${a.university}","${a.college}","${a.stream}","${a.major}","${a.year}","${a.brand}","${a.duration}","${a.status || 'Pending'}"`);
    const csvContent = "data:text/csv;charset=utf-8," + headers.concat(rows).join("\n");
    const link = document.createElement("a"); link.href = encodeURI(csvContent); link.download = `RDS_Applications.csv`;
    document.body.appendChild(link); link.click(); document.body.removeChild(link); showToast("Excel Report Downloaded");
  };
  ...
  const isDuplicate = (val, field) => { if (!val) return false; return activeApplications.filter(a => a[field] === val).length > 1; };
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-body flex">
      ...
      <main className="flex-1 h-screen overflow-y-auto pb-20">
        ...
        <div className="p-8">
          ...
          {activeTab === "dashboard" && (
            <div className="animate-fade-in space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-sm shadow-sm border-l-4 border-corp-blue"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Total Active Applications</div><div className="text-4xl font-display font-black text-gray-900">{activeApplications.length}</div></div>
                <div className="bg-white p-6 rounded-sm shadow-sm border-l-4 border-yellow-400"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Pending Review</div><div className="text-4xl font-display font-black text-gray-900">{activeApplications.filter(a => !a.status || a.status === 'Pending').length}</div></div>
                <div className="bg-white p-6 rounded-sm shadow-sm border-l-4 border-green-500"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Selected Candidates</div><div className="text-4xl font-display font-black text-gray-900">{activeApplications.filter(a => a.status === 'Selected').length}</div></div>
                <div className="bg-white p-6 rounded-sm shadow-sm border-l-4 border-purple-500"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Interviewing</div><div className="text-4xl font-display font-black text-gray-900">{activeApplications.filter(a => a.status === 'Interviewing').length}</div></div>
              </div>
            ...
            <div className="animate-fade-in">
              <div className="bg-white p-4 rounded-sm shadow-sm border border-gray-200 mb-6 flex flex-wrap gap-4 items-center justify-between">
                <div className="flex gap-4 flex-wrap flex-1">
                  ...
                  <select value={filterBatch} onChange={e => setFilterBatch(e.target.value)} className="px-4 py-2 border border-gray-200 rounded-sm text-xs font-bold uppercase tracking-wider bg-gray-50 outline-none">{availableBatches.map(b => <option key={b} value={b}>{b}</option>)}</select>
                  <select value={filterBrand} onChange={e => setFilterBrand(e.target.value)} className="px-4 py-2 border border-gray-200 rounded-sm text-xs font-bold uppercase tracking-wider bg-gray-50 outline-none"><option>All Brands</option><option>MyTripRaja</option><option>MarketerRaja</option></select>
                </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredApps.map(app => (
                      <tr key={app.id} className={`hover:bg-gray-50 ${selectedApps.includes(app.id) ? 'bg-blue-50/30' : ''}`}>
                        ...
                        <td className="px-6 py-4"><div className="text-xs font-bold text-gray-900">{app.college}</div><div className="text-[10px] text-gray-500 mt-1">{app.stream} • {app.major} • {app.gender}</div><div className="mt-2 text-[9px] font-black uppercase tracking-widest text-corp-blue">{app.brand} • {app.duration}</div></td>
                        <td className="px-6 py-4">
                                  ...
                                  <div className="relative group inline-block">
                                    ...
                                    <div className="absolute right-0 bottom-full mb-2 w-64 bg-gray-900 text-white text-[10px] p-3 rounded hidden group-hover:block z-50 shadow-xl max-h-48 overflow-y-auto">
                                      ...
                                      {app.history.slice().reverse().map((h, i) => (
                                        <div key={i} className="mb-2 last:mb-0"><span className="text-gray-400">{h.date}</span><br/>Status changed to <span className="font-bold text-corp-gold">{h.status}</span><br/>by {h.updatedBy}</div>
                                      ))}
                                    </div>
                                  </div>
                                )}
          ...
          {/* RESTORED: TEAM ACCESS TAB IN HR WORKSPACE */}
                   ...
                   <form onSubmit={addStaffMember} className="space-y-4">
                     ...
                     <div><label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Access Role</label>
                        <select value={newStaff.role} onChange={e => setNewStaff({...newStaff, role: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded text-sm outline-none focus:border-corp-blue bg-white">
                          ...
                          <option value="Admin">Admin (Review & Email)</option>
          ...
          {activeTab === "trash" && (
            <div className="animate-fade-in bg-white border border-red-200 rounded-sm shadow-sm overflow-hidden">
               <div className="bg-red-50 p-4 border-b border-red-100"><h3 className="font-bold text-red-800 text-sm">Trash Bin (Soft Deleted Records)</h3><p className="text-xs text-red-600 mt-1">These records are hidden from the main view. Super Admins can permanently erase them.</p></div>
               <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {deletedApplications.map(app => (
                      <tr key={app.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4"><div className="font-mono text-xs font-bold text-gray-900">{app.applicationId}</div><div className="text-sm font-bold text-gray-600 mt-1">{app.name}</div></td>
                        <td className="px-6 py-4 text-xs text-gray-600"><div><span className="font-bold text-gray-900">Deleted By:</span> {app.deletedBy}</div><div><span className="font-bold text-gray-900">Date:</span> {app.deletedAt}</div><div className="mt-2 bg-gray-100 p-2 rounded-sm border border-gray-200 italic">"{app.deleteNote}"</div></td>
                        <td className="px-6 py-4 text-right space-x-3"><button onClick={() => handleRestore(app.id)} className="text-[10px] font-bold text-blue-600 hover:text-blue-800 uppercase tracking-widest bg-blue-50 px-3 py-2 rounded border border-blue-100">Restore</button>{staffData.role === "Super Admin" && (<button onClick={() => handlePermanentDelete(app.id)} className="text-[10px] font-bold text-white uppercase tracking-widest bg-red-600 hover:bg-red-700 px-3 py-2 rounded shadow-sm">Erase</button>)}</td>
                      </tr>
                    ))}
          ...
          {activeTab === "templates" && (
            <div className="animate-fade-in bg-white p-8 rounded-sm shadow-sm border border-gray-200">
              <div className="flex justify-between items-center mb-6">
                <div><h3 className="font-display text-2xl font-bold mb-2">Email Templates</h3><p className="text-sm text-gray-500">Use these tags to auto-fill applicant data: <b className="text-corp-blue">[Name], [Gender], [Phone], [AltPhone], [University], [College], [Stream], [Major], [Batch], [Year], [Duration], [Brand], [LinkedIn], [Portfolio], [Resume]</b>.</p></div>
              ...
              <div className="grid md:grid-cols-2 gap-8">{Object.keys(templates).map(key => (<div key={key}><label className="block text-[10px] font-bold tracking-widest uppercase mb-2 text-gray-700">{key.replace(/([A-Z])/g, ' $1').trim()} Email</label><textarea value={templates[key]} onChange={(e) => setTemplates({...templates, [key]: e.target.value})} className="w-full h-40 p-4 border border-gray-300 rounded-sm text-xs font-body focus:border-corp-blue outline-none resize-none bg-gray-50 focus:bg-white transition-colors" /></div>))}</div>
            </div>
          )}
          
          {activeTab === "enquiries" && (
            <table className="w-full text-left border-collapse bg-white shadow-sm rounded-sm overflow-hidden"><thead><tr className="bg-gray-50 border-b border-gray-200"><th className="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest w-48">Date</th><th className="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest w-64">Sender</th><th className="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest">Message</th></tr></thead><tbody className="divide-y divide-gray-100">{enquiries.map(enq => (<tr key={enq.id} className="hover:bg-gray-50 transition-colors"><td className="px-6 py-4 text-xs text-gray-400 font-bold align-top">{enq.dateStr}</td><td className="px-6 py-4 align-top"><div className="font-bold text-gray-900 mb-2">{enq.name}</div><a href={`mailto:${enq.email}`} className="text-[10px] bg-blue-50 text-blue-700 px-2 py-1 rounded font-black uppercase tracking-widest hover:bg-blue-100">Reply Email</a></td><td className="px-6 py-4 align-top text-sm text-gray-700 whitespace-pre-wrap font-body">{enq.message}</td></tr>))}</tbody></table>
          )}
