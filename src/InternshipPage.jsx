...
export default function InternshipPage() {
  ...
  useEffect(() => {
    const fetchUniversities = async () => {
      try {
        // Attempt to fetch from Hipolabs
        ...
        console.log("Using fallback university list due to network constraints.");
        setUniversitiesList([
          "Anna University", "Bharathiar University", "Madras University", 
          "SRM Institute of Science and Technology", "Vellore Institute of Technology",
          "Delhi University", "Mumbai University" // Add more fallbacks here if needed
        ]);
      }
    };
    fetchUniversities();
  ...
  const processFinalSubmission = async () => {
    if (!agreedToTerms) return; // Safeguard
  ...
  const trackApplication = async (e) => {
    ...
    try {
      ...
      if (querySnapshot.empty) {
        setTrackError("No application found with that ID. Please check and try again.");
      } else {
        const appData = querySnapshot.docs[0].data();
        setTrackingStatus(appData.status || "Pending");
      }
    } catch (error) {
      setTrackError("Error fetching status. Please try again later.");
    }
    ...
    <div className="min-h-screen pt-32 pb-24 relative">
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 sm:p-6">
          <div className="bg-white p-8 rounded-sm shadow-2xl max-w-lg w-full border-t-4 border-corp-red animate-fade-in">
            <h3 className="font-display text-2xl font-black text-gray-900 mb-4">Final Confirmation</h3>
            
            <div className="bg-red-50 text-corp-red text-sm font-bold p-4 rounded-sm border border-red-100 mb-6">
              ⚠️ Once submitted, your application details cannot be edited.
            </div>

            <ul className="text-gray-600 text-sm font-body mb-6 space-y-3 list-disc pl-5">
              <li>We will communicate all updates strictly via the email and phone number you have provided.</li>
              <li>After submission, you will see your unique <b>Application Number</b> on the screen. Please save this number for future reference and to check your status.</li>
            </ul>
            ...
            <div className="flex items-start gap-3 mb-8 bg-gray-50 p-4 border border-gray-200 rounded-sm">
              <input 
                ...
                className="mt-1 w-4 h-4 accent-corp-red cursor-pointer" 
              />
              <label htmlFor="terms" className="text-xs text-gray-700 font-body leading-relaxed cursor-pointer">
                I acknowledge that all information provided is accurate. I have read and agree to the <a href="/terms" target="_blank" className="text-corp-blue hover:underline font-bold">Terms & Conditions</a> and <a href="/privacy" target="_blank" className="text-corp-blue hover:underline font-bold">Privacy Policy</a> of Raja Deepu Sooriya Private Limited.
              </label>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-4xl lg:text-6xl font-black text-white leading-tight mb-8">
            Work. <span className="text-corp-red">Grow.</span> <span className="text-corp-gold">Lead.</span>
          </h1>

          {/* TOGGLE TAB FOR APPLY VS STATUS */}
        ...
        {activeTab === "status" && (
          <div className="max-w-3xl mx-auto bg-white border border-gray-200 rounded-sm p-8 lg:p-12 shadow-xl animate-fade-in">
            ...
            <form onSubmit={trackApplication} className="flex gap-4 mb-10">
              <input 
                type="text" 
                placeholder="e.g. RDS-2026-0001" 
                required
                value={trackingId}
                onChange={(e) => setTrackingId(e.target.value)}
                className="flex-1 px-4 py-3 border border-gray-300 rounded-sm focus:border-corp-blue focus:ring-1 focus:ring-corp-blue outline-none font-mono text-sm uppercase"
              />
              <button type="submit" disabled={isSearching} className="px-8 bg-corp-blue hover:bg-corp-blue-mid text-white font-bold text-xs tracking-widest uppercase rounded-sm transition-colors shadow-sm">
              ...
              <div className="mt-12 pt-8 border-t border-gray-100">
                ...
                <div className="relative flex flex-col sm:flex-row justify-between items-center sm:items-start gap-8 sm:gap-0">
                  ...
                  <div className="flex flex-col items-center text-center relative w-full sm:w-1/4">
                    ...
                    <div className="text-sm font-bold text-gray-900">Received</div>
                    <div className="text-[10px] text-gray-500 mt-1">Application Submitted</div>
                  </div>
                  ...
                  <div className={`flex flex-col items-center text-center relative w-full sm:w-1/4 ${['Reviewed', 'Interviewing', 'Selected'].includes(trackingStatus) ? 'opacity-100' : 'opacity-40'}`}>
                    ...
                    <div className="text-sm font-bold text-gray-900">Under Review</div>
                    <div className="text-[10px] text-gray-500 mt-1">HR Screening</div>
                  </div>
                  ...
                  <div className={`flex flex-col items-center text-center relative w-full sm:w-1/4 ${['Interviewing', 'Selected'].includes(trackingStatus) ? 'opacity-100' : 'opacity-40'}`}>
                    ...
                    <div className="text-sm font-bold text-gray-900">Interviewing</div>
                    <div className="text-[10px] text-gray-500 mt-1">Shortlisted</div>
                  </div>
                  ...
                  <div className={`flex flex-col items-center text-center relative w-full sm:w-1/4 ${['Selected', 'Rejected'].includes(trackingStatus) ? 'opacity-100' : 'opacity-40'}`}>
                    ...
                    <div className="text-sm font-bold text-gray-900">{trackingStatus === 'Rejected' ? 'Not Selected' : 'Final Decision'}</div>
                    <div className="text-[10px] text-gray-500 mt-1">{trackingStatus === 'Selected' ? 'Offer Extended' : 'Process Closed'}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="bg-[#f4f7fb] border border-gray-200 rounded-sm p-6 sm:p-8 shadow-xl relative">
              {sentAppId ? (
                <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center">
                  ...
                  <button onClick={() => {setSentAppId(null); setActiveTab("status");}} className="text-xs text-corp-blue font-bold tracking-widest uppercase hover:underline">Track Application Status →</button>
                </div>
              ) : !verifiedEmail ? (
                <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center">
                  ...
                  <div className="space-y-4 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="sm:col-span-2">
                        <label className="flex items-center text-gray-900 text-[10px] font-bold tracking-widest uppercase mb-2">
                          ...
                          <div className="relative group inline-block ml-2">
                            ...
                            <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 bg-gray-900 text-white text-[10px] p-2 rounded hidden group-hover:block z-10 normal-case tracking-normal">Enter the University that your specific college comes under (e.g., Anna University, Bharathiar University).</div>
                          </div>
                        </label>
                        <input 
                          type="text" required list="universities-list" placeholder="Search your university..." 
                          value={form.university} onChange={e => setForm({ ...form, university: e.target.value })} 
                          className="w-full px-3 py-2.5 border border-gray-300 bg-white rounded-sm focus:border-corp-blue focus:ring-1 focus:ring-corp-blue text-sm outline-none transition-all" 
                        />
                      </div>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <input 
                          type="text" required placeholder="Type your specific college name..." 
                          value={form.college} onChange={e => setForm({ ...form, college: e.target.value })} 
                          className="w-full px-3 py-2.5 border border-gray-300 bg-white rounded-sm focus:border-corp-blue focus:ring-1 focus:ring-corp-blue text-sm outline-none transition-all" 
                        />
                        <p className="text-[10px] text-gray-500 mt-1 italic">Note: If you study directly at the University campus, you can enter the University name again here.</p>
                      </div>
                      
                      <div>
                        <label className="block text-gray-900 text-[10px] font-bold tracking-widest uppercase mb-2">Degree / Stream *</label>
                        <input 
                          type="text" required placeholder="e.g. B.Com, B.E., MBA..." 
                      ...
                      <div>
                        ...
                        <input type="text" required placeholder="e.g. Finance, IT, None" value={form.major} onChange={e => setForm({ ...form, major: e.target.value })} className="w-full px-3 py-2.5 border border-gray-300 bg-white rounded-sm focus:border-corp-blue focus:ring-1 focus:ring-corp-blue text-sm outline-none transition-all" />
                      </div>
