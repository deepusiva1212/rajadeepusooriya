  ...
  if (!user || !staffData) return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center font-body p-6">
      <div className="bg-white p-12 rounded-sm shadow-2xl w-full max-w-md border-t-4 border-corp-gold text-center">
        ...
        <button onClick={() => signInWithPopup(auth, provider)} className="w-full py-4 mt-6 bg-corp-gold hover:bg-yellow-600 text-white font-bold text-xs tracking-widest uppercase rounded-sm">Director Login</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-100 font-body flex">
      ...
      <aside className="w-64 bg-gray-900 text-white hidden md:flex flex-col shadow-2xl z-20">
        ...
        <div className="p-6 border-t border-white/10">
          <button onClick={() => signOut(auth)} className="w-full py-2 border border-white/20 text-gray-300 hover:bg-white/10 text-[10px] font-bold uppercase rounded-sm">Sign Out</button>
        </div>
      </aside>
      ...
      <main className="flex-1 h-screen overflow-y-auto p-8">
             ...
             <div className="lg:col-span-1">
               <div className="bg-white p-6 border border-gray-200 rounded-sm shadow-sm">
                 ...
                 <form onSubmit={handleAssignTask} className="space-y-4">
                   ...
                   <button type="submit" className="w-full py-3 bg-gray-900 hover:bg-black text-corp-gold font-bold text-[10px] uppercase tracking-widest rounded">Issue Task</button>
                 </form>
               </div>
             </div>
             <div className="lg:col-span-2">
               <div className="bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden">
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead><tr className="border-b border-gray-200"><th className="px-6 py-3 text-[10px] text-gray-500 uppercase">Employee</th><th className="px-6 py-3 text-[10px] text-gray-500 uppercase">Task Details</th><th className="px-6 py-3 text-[10px] text-gray-500 uppercase">Status</th></tr></thead>
                    <tbody className="divide-y divide-gray-100">
                      {tasks.map(task => (
                        <tr key={task.id} className="hover:bg-gray-50"><td className="px-6 py-4 font-bold text-sm text-gray-900">{task.assignedToName}</td><td className="px-6 py-4"><div className="font-bold text-sm text-gray-900 mb-1">{task.title}</div><div className="text-[10px] font-bold text-red-600 uppercase">Due: {task.deadline}</div></td><td className="px-6 py-4"><div className={`inline-block px-3 py-1 rounded text-[10px] font-black uppercase ${task.status === 'Pending' ? 'bg-yellow-100 text-yellow-700' : task.status === 'In Progress' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>{task.status}</div></td></tr>
                      ))}
                    </tbody>
                  </table>
               </div>
        ...
        {activeTab === "leave-approvals" && (
          <div className="animate-fade-in bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden">
             ...
             <table className="w-full text-left border-collapse">
                <thead><tr className="border-b border-gray-200"><th className="px-6 py-3 text-[10px] text-gray-500 uppercase">Employee</th><th className="px-6 py-3 text-[10px] text-gray-500 uppercase">Details</th><th className="px-6 py-3 text-[10px] text-gray-500 uppercase text-right">Director Decision</th></tr></thead>
                <tbody className="divide-y divide-gray-100">
                  {leaves.map(leave => (
                    <tr key={leave.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4"><div className="font-bold text-sm text-gray-900">{leave.applicantName}</div><div className="text-[10px] text-gray-500">{leave.applicantEmail}</div></td>
                      <td className="px-6 py-4"><div className="font-bold text-sm text-gray-900 mb-1">{leave.startDate} to {leave.endDate}</div><div className="text-[10px] font-black uppercase text-gray-500 mb-2">{leave.type}</div><div className="text-xs text-gray-600 italic">"{leave.reason}"</div></td>
                      <td className="px-6 py-4 text-right">
                        {leave.status === 'Pending' ? (
                          <div className="flex gap-2 justify-end"><button onClick={() => updateLeaveStatus(leave.id, 'Approved')} className="text-[10px] font-bold text-green-700 bg-green-50 px-3 py-1.5 rounded uppercase">Approve</button><button onClick={() => updateLeaveStatus(leave.id, 'Rejected')} className="text-[10px] font-bold text-red-700 bg-red-50 px-3 py-1.5 rounded uppercase">Reject</button></div>
                        ) : (
             ...
             <div className="lg:col-span-1">
               <div className="bg-white p-6 border border-gray-200 rounded-sm shadow-sm">
                 ...
                 <form onSubmit={addStaffMember} className="space-y-4">
                   ...
                   <button type="submit" className="w-full py-3 bg-gray-900 hover:bg-black text-corp-gold font-bold text-[10px] uppercase rounded">Generate ID</button>
                 </form>
               </div>
             </div>
             <div className="lg:col-span-2">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {staffList.map(staff => (
                        <tr key={staff.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4"><div className="font-bold text-sm text-gray-900">{staff.name}</div><div className="text-[10px] text-gray-500">{staff.email}</div></td>
                          ...
                          <td className="px-6 py-4 text-right">{staff.email !== user.email && <button onClick={() => removeStaffMember(staff.id)} className="text-[10px] font-bold text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded uppercase">Revoke</button>}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
