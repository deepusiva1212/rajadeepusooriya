             ...
             <div className="lg:col-span-1">
               <div className="bg-white p-6 border border-gray-200 rounded-sm shadow-sm">
                 ...
                 <form onSubmit={handleAssignTask} className="space-y-4">
                   <div><label className="block text-[10px] font-bold uppercase text-gray-500 mb-1">Assign To</label><select required value={newTask.assignedToEmail} onChange={e => setNewTask({...newTask, assignedToEmail: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded text-sm bg-white outline-none"><option value="">Select Employee...</option>{staffList.filter(s => s.role === "User").map(s => <option key={s.email} value={s.email}>{s.name} (Employee)</option>)}</select></div>
